...
const createSingle = async (doc) => {
  // CREATE DOC IN DB
  let createdDoc = await prisma.user.create({
    data: doc,
...
const updateSingle = async (doc) => {
  ...
  const id = doc.id
  delete doc.id
  const updatedDoc = await prisma.user.update({
    where: {
      id: id
    },
    data: doc
...
const getMany = async () => {
  // GET DOCS FROM DB
  let docs = await prisma.user.findMany()  // console.log("RETURN", docs) // [docs] array of docs
  // RETURN DOCS
  return docs
} 
